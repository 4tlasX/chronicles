...
// Post validation
const metadataSchema = z.record(z.unknown()).optional().refine((val) => !val || JSON.stringify(val).length <= 10000, { message: 'Metadata too large (max 10KB)' });
...
export const upsertSettingSchema = z.object({
    ...
    value: z.unknown(),
}).refine((data) => JSON.stringify(data.value).length <= 65536, { message: 'Setting value too large (max 64KB)', path: ['value'] });
