...
import { generateMasterKey, generateRecoveryKey, generateSalt, generateIv, deriveKEK, importRawKey, wrapKey, unwrapKey, toNonExtractable, encrypt, decrypt, } from './primitives.js';
...
class EncryptionService {
     ...
     */
    async unwrapMasterKey(password, saltBase64, wrappedMKBase64, wrapIvBase64, iterations) {
        ...
        return unwrapKey(wrappedMK.buffer, kek, wrapIv, false, 'kek-wrap');
    }
    /**
     ...
     */
    async unwrapWithRecoveryKey(recoveryKeyBase64, recoveryWrappedMKBase64, recoveryWrapIvBase64) {
        ...
        const recoveryKeyObj = await importRawKey(recoveryKeyBytes);
        return unwrapKey(recoveryWrappedMK.buffer, recoveryKeyObj, recoveryWrapIv, true, 'recovery-wrap');
    }
    /**
     ...
     */
    async rewrapFromParams(currentPassword, kekSaltBase64, encryptedMKBase64, kekWrapIvBase64, kekIterations, newPassword) {
        ...
        const extractableKey = await unwrapKey(wrappedMK.buffer, kek, wrapIv, true, 'kek-wrap');
        try {
     ...
     */
    async encryptPost(masterKey, content, metadata) {
     ...
     */
    async decryptPost(masterKey, post) {
        if (!post.isEncrypted) {
            return {
                id: post.id,
     ...
     */
    async decryptPosts(masterKey, posts) {
