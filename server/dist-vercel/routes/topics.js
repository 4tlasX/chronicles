...
const escapeSchema_js_1 = require("../db/escapeSchema.js");
/** JIT migration: add sort_order column if missing (for schemas created before this feature). */
async function ensureSortOrderColumn(schemaName) {
    ...
    if (!result[0]?.exists) {
        await prisma_js_1.prisma.$executeRawUnsafe(`ALTER TABLE ${s}.taxonomies ADD COLUMN sort_order INTEGER DEFAULT 0`);
    }
}
...
async function ensureDefaultTopics(schemaName) {
    const existing = await (0, tenantQueries_js_1.getAllTaxonomies)(schemaName);
    const existingNames = new Set(existing.map(t => t.name.toLowerCase()));
    const missing = DEFAULT_TOPICS.filter(t => !existingNames.has(t.name.toLowerCase()));
    if (missing.length === 0)
        return;
    for (const topic of missing) {
        await (0, tenantQueries_js_1.createTaxonomy)(schemaName, topic.name, { icon: topic.icon, color: topic.color });
    }
}
// GET /api/topics
router.get('/', async (req, res) => {
    try {
        ...
        const taxonomies = await (0, tenantQueries_js_1.getAllTaxonomies)(req.auth.tenantSchemaName);
        res.json(taxonomies);
    }
    catch (err) {
...
router.post('/reorder', async (req, res) => {
    try {
        ...
        await (0, tenantQueries_js_1.reorderTaxonomies)(req.auth.tenantSchemaName, topicIds);
...
router.post('/', async (req, res) => {
    try {
        const parsed = shared_1.createTaxonomySchema.safeParse(req.body);
        if (!parsed.success) {
            res.status(400).json({ error: parsed.error.errors[0].message });
            return;
        }
        const { name, icon, color } = parsed.data;
        const taxonomy = await (0, tenantQueries_js_1.createTaxonomy)(req.auth.tenantSchemaName, name, { icon, color });
...
router.get('/:id', async (req, res) => {
    try {
        ...
        const taxonomy = await (0, tenantQueries_js_1.getTaxonomy)(req.auth.tenantSchemaName, id);
        if (!taxonomy) {
...
router.put('/:id', async (req, res) => {
    try {
        ...
        const parsed = shared_1.updateTaxonomySchema.safeParse(req.body);
        if (!parsed.success) {
            res.status(400).json({ error: parsed.error.errors[0].message });
            return;
        }
        const taxonomy = await (0, tenantQueries_js_1.updateTaxonomy)(req.auth.tenantSchemaName, id, parsed.data);
        res.json(taxonomy);
    }
    catch (err) {
...
router.delete('/:id', async (req, res) => {
    try {
        ...
        await (0, tenantQueries_js_1.deleteTaxonomy)(req.auth.tenantSchemaName, id);
        res.json({ success: true });
    }
    catch (err) {
