...
const schemaManager_js_1 = require("../db/schemaManager.js");
...
async function constantTimeDelay(startTime, minMs = 200) {
...
router.post('/register', rateLimiter_js_1.authLimiter, async (req, res) => {
    try {
        const parsed = shared_1.registerSchema.safeParse(req.body);
        if (!parsed.success) {
        ...
        if (existing) {
            (0, securityLogger_js_1.logSecurityEvent)('register_duplicate', { ip: req.ip });
            // Constant-time delay to prevent timing-based enumeration
            await bcryptjs_1.default.hash('dummy', BCRYPT_ROUNDS);
        ...
        const { account, schemaName } = await (0, schemaManager_js_1.registerTenant)(email, username, passwordHash, {
        ...
        // Create session
        const token = await (0, auth_js_1.createSession)(account.id, schemaName, {
            ipAddress: req.ip,
            userAgent: req.headers['user-agent'],
        });
        (0, securityLogger_js_1.logSecurityEvent)('register_success', { accountId: account.id, ip: req.ip });
...
router.post('/login', rateLimiter_js_1.authLimiter, async (req, res) => {
    ...
    try {
        const parsed = shared_1.loginSchema.safeParse(req.body);
        if (!parsed.success) {
        ...
        const account = await prisma_js_1.prisma.account.findUnique({ where: { email: normalizedEmail } });
        if (!account) {
            (0, securityLogger_js_1.logSecurityEvent)('login_failed_no_account', { ip: req.ip });
        ...
        if (!valid) {
            (0, securityLogger_js_1.logSecurityEvent)('login_failed_bad_password', { accountId: account.id, ip: req.ip });
            await constantTimeDelay(startTime);
            res.status(401).json({ error: 'Invalid credentials' });
            return;
        }
        const token = await (0, auth_js_1.createSession)(account.id, account.tenantSchemaName, {
            ipAddress: req.ip,
            userAgent: req.headers['user-agent'],
        });
        (0, securityLogger_js_1.logSecurityEvent)('login_success', { accountId: account.id, ip: req.ip });
        res.cookie(COOKIE_NAME, token, COOKIE_OPTIONS);
        res.json({
            user: { email: account.email, username: account.username },
            encryption: {
...
// =============================================================================
router.post('/logout', auth_js_1.authMiddleware, async (req, res) => {
    try {
        await (0, auth_js_1.revokeSession)(req.auth.selector, 'user_logout');
        (0, securityLogger_js_1.logSecurityEvent)('logout', { accountId: req.auth.accountId, ip: req.ip });
        res.clearCookie(COOKIE_NAME, { path: '/', secure: IS_PRODUCTION, sameSite: 'strict' });
...
// =============================================================================
router.get('/salt', rateLimiter_js_1.authLimiter, async (req, res) => {
    const startTime = Date.now();
    try {
        ...
        const emailParsed = shared_1.emailSchema.safeParse(rawEmail);
        ...
        if (!account) {
            ...
            const fakeSalt = crypto_1.default.randomBytes(16).toString('base64');
            const fakeWrappedKey = crypto_1.default.randomBytes(48).toString('base64');
            const fakeIv = crypto_1.default.randomBytes(12).toString('base64');
            await constantTimeDelay(startTime);
            res.json({
                encryptionEnabled: true,
                kekSalt: fakeSalt,
...
// =============================================================================
router.post('/change-password', rateLimiter_js_1.strictLimiter, auth_js_1.authMiddleware, async (req, res) => {
    try {
        const parsed = shared_1.changePasswordSchema.safeParse(req.body);
        if (!parsed.success) {
            res.status(400).json({ error: 'Password does not meet requirements' });
            return;
        }
        const { currentPassword, newPassword, newEncryptedMasterKey, newKekSalt, newKekWrapIv } = parsed.data;
        const account = await prisma_js_1.prisma.account.findUnique({ where: { id: req.auth.accountId } });
        if (!account) {
            res.status(404).json({ error: 'Account not found' });
            return;
        }
        const valid = await bcryptjs_1.default.compare(currentPassword, account.passwordHash);
        if (!valid) {
            (0, securityLogger_js_1.logSecurityEvent)('password_change_failed', { accountId: account.id, ip: req.ip });
        ...
        await prisma_js_1.prisma.$transaction([
            prisma_js_1.prisma.account.update({
                where: { id: account.id },
                data: {
                    passwordHash,
            ...
            prisma_js_1.prisma.session.updateMany({
                where: {
        ...
        (0, securityLogger_js_1.logSecurityEvent)('password_change', { accountId: account.id, ip: req.ip });
        res.cookie(COOKIE_NAME, token, COOKIE_OPTIONS);
        res.json({ success: true });
    }
    catch (err) {
...
router.post('/change-email', async (req, res) => {
    try {
        ...
        const existing = await prisma_js_1.prisma.account.findUnique({ where: { email: normalizedEmail } });
...
// =============================================================================
router.post('/recover', rateLimiter_js_1.strictLimiter, async (req, res) => {
    const startTime = Date.now();
    try {
        const parsed = shared_1.recoverSchema.safeParse(req.body);
        if (!parsed.success) {
            res.status(400).json({ error: 'Password does not meet requirements' });
            return;
        }
        ...
        const email = rawEmail.toLowerCase();
        const account = await prisma_js_1.prisma.account.findUnique({ where: { email } });
        if (!account) {
            // Constant-time delay to prevent timing-based enumeration
            await bcryptjs_1.default.hash('dummy', BCRYPT_ROUNDS);
            await constantTimeDelay(startTime);
        ...
        if (!account.recoveryKeyHash || !account.recoveryKeySalt) {
            ...
            (0, securityLogger_js_1.logSecurityEvent)('recovery_failed', { accountId: account.id, ip: req.ip, details: { reason: 'no_recovery_hash' } });
            await constantTimeDelay(startTime);
            res.status(401).json({ error: 'Recovery failed' });
            return;
        }
        ...
        const providedHash = crypto_1.default.pbkdf2Sync(recoveryKey, recoveryKeySalt, 600000, 32, 'sha256').toString('hex');
        if (!crypto_1.default.timingSafeEqual(Buffer.from(providedHash), Buffer.from(account.recoveryKeyHash))) {
            (0, securityLogger_js_1.logSecurityEvent)('recovery_failed', { accountId: account.id, ip: req.ip });
            await constantTimeDelay(startTime);
            res.status(401).json({ error: 'Recovery failed' });
            return;
        }
        ...
        const txResult = await prisma_js_1.prisma.$transaction(async (tx) => {
            // Lock the account row to prevent concurrent recovery attempts
            const locked = await tx.$queryRawUnsafe('SELECT id, recovery_key_hash FROM "Account" WHERE id = $1 FOR UPDATE', account.id);
        ...
        if (!txResult) {
            (0, securityLogger_js_1.logSecurityEvent)('recovery_failed', { accountId: account.id, ip: req.ip, details: { reason: 'concurrent_recovery' } });
            await constantTimeDelay(startTime);
            res.status(401).json({ error: 'Recovery failed' });
            return;
        }
        (0, securityLogger_js_1.logSecurityEvent)('recovery_success', { accountId: account.id, ip: req.ip });
...
// =============================================================================
router.get('/recovery-params', rateLimiter_js_1.authLimiter, async (req, res) => {
    const startTime = Date.now();
    try {
        const rawEmail = req.query.email;
        if (!rawEmail || typeof rawEmail !== 'string' || rawEmail.length > 254) {
